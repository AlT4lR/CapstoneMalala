...
# =========================================================
# ADD TRANSACTION (REVERTED TO AUTO-CALCULATION LOGIC)
# =========================================================
def add_transaction(username, branch, transaction_data, parent_id=None):
    ...
    try:
        ...
        else: # This is a CHILD CHECK (Logic reverted)
...
# =========================================================
# UPDATE CHILD TRANSACTION (REVERTED TO AUTO-CALCULATION LOGIC)
# =========================================================
...
# (The rest of the functions in this file are not modified)
def get_transaction_by_id(username, transaction_id, full_document=False):
    ...
    try:
        ...
        check_date_str = doc['check_date'].strftime('%Y-%m-%d') if doc.get('check_date') else ''
        due_date_str = doc['due_date'].strftime('%Y-%m-%d') if doc.get('due_date') else ''
        return {'_id': str(doc['_id']), 'name': doc.get('name'), 'check_no': doc.get('check_no'), 'check_date': check_date_str, 'due_date': due_date_str, 'check_amount': doc.get('check_amount', 0.0), 'deductions': doc.get('deductions', []), 'ewt': doc.get('ewt', 0.0), 'countered_check': doc.get('countered_check', 0.0), 'amount': doc.get('amount', 0.0), 'notes': doc.get('notes', ''), 'status': doc.get('status')}
    except Exception as e:
...
def get_transactions_by_status(username, branch, status):
    ...
    try:
        ...
        for doc in db.transactions.find(query).sort('check_date', -1):
            ...
            check_date_str = doc['check_date'].strftime('%m/%d/%Y') if doc.get('check_date') else 'N/A'
            due_date_str = doc['due_date'].strftime('%m/%d/%Y') if doc.get('due_date') else 'N/A'
...
def get_child_transactions_by_parent_id(username, parent_id):
    ...
    child_checks = []
    try:
        query = {'username': username, 'parent_id': ObjectId(parent_id), '$or': [{'isArchived': {'$exists': False}}, {'isArchived': False}]}
...
def update_transaction(username, transaction_id, form_data):
    ...
    try:
        ...
        check_date_obj = form_data.get('check_date')
        if check_date_obj: update_fields['check_date'] = pytz.utc.localize(datetime.combine(check_date_obj, datetime.min.time()))
        due_date_obj = form_data.get('due_date')
        if due_date_obj: update_fields['due_date'] = pytz.utc.localize(datetime.combine(due_date_obj, datetime.min.time()))
