...
// These global variables are expected to be set by the Jinja2 template in schedules.html
// const FLASK_INITIAL_DATE_ISO;
// const FLASK_INITIAL_CATEGORIES_JSON; // This will be a JSON string of categories
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentMainDate; // Tracks the date currently displayed in the main calendar header (could be day, week, or month start)
    ...
    const categoryColors = {
        ...
        // Add more dynamic categories here if needed from backend
    };
    ...
    async function fetchCategories() {
        try {
            ...
            return categories.length > 0 ? categories : ['Office', 'Meetings', 'Events', 'Personal', 'Others']; // Fallback
        } catch (error) {
    ...
    async function populateCategorySelectAndList(initialCategories = null) {
        // Use FLASK_INITIAL_CATEGORIES_JSON for initial load, then fetch for updates
        ...
        categories.forEach(cat => {
            // Populate Create Schedule modal's category select
            ...
            // Ensure the checkbox/radio button has a unique ID if it needs to be accessible
            // For categories that might not have a direct Tailwind color mapping, use a default or dynamic color
            const categoryStyle = `color: ${getComputedStyle(document.documentElement).getPropertyValue(`--tw-colors-${cat.toLowerCase()}`) || 'gray'};`; // Attempt to get Tailwind color or default
            
            label.innerHTML = `
                <input type="radio" name="sidebar-category" value="${cat}" class="form-radio h-4 w-4" style="${categoryStyle}">
                <span class="ml-2">${cat}</span>
            `;
            // Add a temporary mapping for dynamically added categories, for client-side display
            if (!categoryColors[cat]) {
                 categoryColors[cat] = `bg-random-color-${Math.floor(Math.random() * 100)}`; // Placeholder random class
                 // You'd need to define these in tailwind.config.js or via inline style directly on the event box
                 // For now, rely on default if class not found.
            }
    ...
    function renderMonthView(startDate, events) {
        ...
        for (let i = 0; i < 42; i++) { // 6 weeks * 7 days
            ...
            dailyEvents.slice(0, 3).forEach(event => { // Display max 3 events
                ...
                eventEl.className = `event-box multi-day ${getCategoryColorClass(event.category)}`;
    ...
    function renderWeekView(startDate, events) {
        ...
        // For week view, events need to be placed precisely based on time and duration
    ...
    addCategoryBtn.addEventListener('click', async () => {
        ...
        if (newCategory && newCategory.trim() !== "") {
            try {
                ...
                if (response.ok) {
                    alert(`Category '${newCategory.trim()}' added! (Note: This is conceptual. Actual persistence depends on backend implementation)`);
                    // Dynamically add to current categoryColors for immediate use (client-side only for this example)
                    // In a real app, you might fetch categories again or send a specific color from backend.
                    // For now, assign a random color for demonstration.
                    const randomHue = Math.floor(Math.random() * 360);
                    const newCategoryColorClass = `bg-[hsl(${randomHue},_70%,_70%)]`; // Tailwind JIT color
    ...
    // --- Initial Render ---
    // Parse the initial categories provided by Flask
    const initialCategories = JSON.parse(FLASK_INITIAL_CATEGORIES_JSON);
    populateCategorySelectAndList(initialCategories); // Load initial categories
    renderMiniCalendar();
