// static/js/index.js (or common.js)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (notificationBtn && notificationPanel) {
        ...
        document.addEventListener('click', (event) => {
            ...
            // Ensure the panel and the button are not the target of the click
    ...
    async function getCsrfToken() {
        // First, try to get it from a meta tag (most common and recommended)
        const csrfTokenMeta = document.querySelector('meta[name="csrf-token"]');
        if (csrfTokenMeta) {
            return csrfTokenMeta.getAttribute('content');
        }

        // If not found in meta tag, try to retrieve it from a cookie.
        // This assumes Flask-JWT-Extended or Flask-WTF sets a cookie named 'csrf_access_token' or similar.
        // Adjust the cookie name if your setup uses a different one.
    ...
    window.fetch = async function(url, options = {}) {
        // Only add CSRF token for requests to our API endpoints
        // and only for methods that typically require CSRF protection (POST, PUT, DELETE)
        if (url.startsWith('/api/') && options.method && !['GET', 'HEAD'].includes(options.method.toUpperCase())) {
            ...
            if (csrfToken) {
                options.headers = {
                    ...
                    // Ensure Content-Type is set for POST/PUT/etc. requests, especially if not explicitly set
    ...
    // Example for createScheduleForm in schedules.html
        ...
        createScheduleForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('/api/schedules', {
                    ...
                    body: JSON.JSON.stringify(data), // Use JSON.stringify for the body
                });
    ...
    // Add similar CSRF handling for other JS-driven form submissions if any.


    // --- Calendar Functionality (if this is indeed index.js and not common.js) ---
    // If this file is purely for common JS, the calendar logic should be in a separate file
    // and included on the schedules.html page specifically.
    // For now, assuming this file contains BOTH common JS and calendar JS if not using base.html properly.

    // If you have a separate index.js for calendar logic, ensure it is included only on schedules.html
    // and that it also uses the CSRF token handling for its fetch calls.
    // The logic below is specific to schedules.html and should ideally be in its own file.
    ...
    if (document.getElementById('create-schedule-btn') && 
        ...
        // createScheduleForm is already handled above for CSRF
            ...
            if (newCategory && newCategory.trim() !== "") {
                try {
                    ...
                    if (response.ok) {
                        ...
                        // For demonstration, dynamically add to categoryColors if not present
                        if (!categoryColors[newCategory.trim()]) {
                             const randomHue = Math.floor(Math.random() * 360);
                             const newCategoryColorClass = `bg-[hsl(${randomHue},_70%,_70%)]`; // Example dynamic color
